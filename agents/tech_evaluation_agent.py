...
def tech_evaluation_node(state: InvestmentState) -> InvestmentState:
    ...
    payload = json_response(
        load_prompt("tech_evaluation.txt"),
        {
            ...
            "research_summary": state.get("tech_research_summary", state.get("research_summary", "")),
            "research_snippets": state.get("tech_research_snippets", state.get("research_snippets", []))[:5],
        },
    )
    assessment = {
